    ...
    for i = 1:size(vol,2)
        for j = 1:size(vol,3)
            ...
            if max(aline) > 0.05
                ...
                fun_pix = @(p,zdata)double(sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-((zf-sur(i,j))*Z_step+i*20/size(vol,2)*zf_tilt*Z_step))./70).^2)))); 
                lb = [0.0001 0.0001 ];
                ub=[20 0.03 ];
                try
        ...
        for j = 1:size(vol,3)
            ...
            if max(aline) > 0.05                                                                % remove agarose
                ...
                param(i,j,3)=(...
                    ...
                    + i*ds_factor/size(vol,2)*zf_tilt*Z_step...                                 % zf curvature across FOV, using linear curvature
                    /(musPrefit(ceil(i/20*ds_factor),ceil(j/20*ds_factor))/20+1)...             % mus caused zf curvature shift
                    ); % NC6839
                ...
                try
                   ...
                   % pause here, use line 144 to check the fitting quality
