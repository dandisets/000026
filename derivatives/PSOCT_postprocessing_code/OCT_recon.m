...
P2path = '/projectnb2/npbssmic/ns/Ann_Mckee_samples_10T/AD_10382_2P/';  % 2P data path.
...
% Use 1 when there is no depth scanning during imaging, i.e., image twice for 450um slice
...
stitch_slice1=19;    % if stitch == 1, first slice for stitching                                ADJUST FOR EACH SAMPLE!!!
stitch_slice2=39;   % if stitch == 1, second slice for stitching                                ADJUST FOR EACH SAMPLE!!!
stitch_slice3=69;   % if stitch == 1, third slice for stitching                                ADJUST FOR EACH SAMPLE!!!
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
istop=0;%section; % end time number for each parallel task          !!!!!!!!!!!!!!  IF WANT TO REDO STITCHING WITHOUT REPEATING DISTORTION CORRECTION, SET ISTOP=0
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% FOR LOOP TO PERFORM DISTORTION CORRECTION
for islice=id
    cd(OCTpath)
...
%     BKG_stitch('BKG', P2path, datapath,disp,mosaic,pxlsize./10,islice,pattern,sys,stitch);
    ...
    BaSiC_shading_and_ref_stitch(islice,P2path,OCTpath, numX*numY, start_pixel, slice_thickness,stitch,aip_threshold_post_BaSiC, pxlsize, Xoverlap, Yoverlap);  % volume recon 
...
%    delete log*.txt
   Concat_ref_vol(nslice,OCTpath, aip_threshold_post_BaSiC, remove_agar, ten_ds);
    if ten_ds == 0
        ref_mus(OCTpath, nslice, nslice*11, 50,50, ten_ds); % volume intensity correction, comment the mus part if no fitting is generated
    else
        ref_mus(OCTpath, nslice, nslice*5, 20,20, ten_ds); % volume intensity correction, comment the mus part if no fitting is generated
    end
end
system(['chmod -R 777 ',OCTpath]);