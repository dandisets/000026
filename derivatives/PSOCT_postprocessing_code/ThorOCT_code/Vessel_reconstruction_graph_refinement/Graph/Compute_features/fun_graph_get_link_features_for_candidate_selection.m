function lf = fun_graph_get_link_features_for_candidate_selection(link_cc, vessel_image, vessel_mask_dt, link_feature)
...
if ~isempty(vessel_image)
    ...
    if ~isempty(vessel_mask_dt) && ismember('bg', link_feature)
        vessel_mask = vessel_mask_dt > 0;
    end
elseif ~isempty(vessel_mask_dt) && ismember('bg', link_feature)
...
lf.int_min_from_mid = zeros(num_l, 1);
...
% Difference between the distance transform at the two ends / length of the
% vessel
lf.dt_e2e_2_length = lf.dt_diff_ep2ep ./ lf.length;
lf.dt_max_2_length = lf.dt_max ./ lf.length;
% Range of distance transform / length of the vessel
