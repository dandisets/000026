function gap_linking_str = fun_graph_connect_gap_p2p_no_backtrack(vessel_image, vessel_mask, p_1_pos, p_2_pos, connectivity)
...
% Date: 01, 09, 2019
% To do: 
% 1. Prevent back tracking: the found linker travels along the vessel,
% being very closed to the existing skeleton. The way to do this can be: 
%   a. By comparing the distance transform of the link voxel vs the
%   distance to the endpoint - The way previously used. - Need to input the
%   vessel graph or the link voxel list that the two points ( not necessary
%   endpoints) belong to 
%   b. Add panelty on the distance transform of the vessel mask. 
if nargin < 4
    connectivity = 26;
    vessel_mask = [];
elseif nargin < 5
...
search_metric = fun_graph_get_shortest_path_distance_metric(local_int, connected_mask);
