function connect_link_str = fun_graph_shortest_path_through_3pts(metric,  ep1_sub, mid_sub, ep2_sub, connectivity)
...
mid_ind = sub2ind(local_bbox_size, mid_sub(1), mid_sub(2), mid_sub(3));
...
linker_ep2mid = shortestpath(local_graph, ep1_ind, mid_ind, 'Method', 'positive');
% Remove the found path from the graph 
tmp_s = [];
tmp_t = [];
if numel(linker_ep2mid) > 1
    tmp_s = linker_ep2mid(1:end-1);
    tmp_t = linker_ep2mid(2:end);
end
if ~isempty(tmp_s)
    local_graph = rmedge(local_graph, tmp_s, tmp_t);
end
linker_mid2ep = shortestpath(local_graph, mid_ind, ep2_ind, 'Method', 'positive');

connect_link_str.link_ind_w_ep = cat(2, linker_ep2mid, linker_mid2ep(2:end))';
