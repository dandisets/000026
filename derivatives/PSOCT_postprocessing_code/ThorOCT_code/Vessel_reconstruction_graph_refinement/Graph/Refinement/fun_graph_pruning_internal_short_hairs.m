function [input_graph, varargout] = fun_graph_pruning_internal_short_hairs(input_graph, th_hair_length, internal_offset)
...
num_dim = numel(block_size);
if ~isfield(input_graph.num, 'neighbor_add')
    input_graph.num.neighbor_add = fun_skeleton_neighbor_add_coeff_3D(block_size, 26, true);
end
internal_bbox = [ones([1,num_dim])*internal_offset, block_size - internal_offset + 1];
endpoint_pos_sub = fun_ind2sub(input_graph.num.mask_size, input_graph.endpoint.pos_ind);

endpoint_internal_Q = all(bsxfun(@gt, endpoint_pos_sub, ...
    internal_bbox(1:num_dim)),2) & all(bsxfun(@lt, endpoint_pos_sub, ...
    internal_bbox((num_dim+1):(2*num_dim))),2);
...
%     if input_graph.tmp_link_ind_old_to_new(link_label + 1) ~= link_label
%         fprintf('Link label changed. Idx %d Original label %d Curretn label %d\n', int_ep_idx, link_label, input_graph.tmp_link_ind_old_to_new(link_label + 1));
%         fprintf('Original length %d Current length %d\n', input_graph.link.num_voxel_per_cc(link_label), input_graph.link.num_voxel_per_cc(input_graph.tmp_link_ind_old_to_new(link_label + 1)));
%         link_label = input_graph.tmp_link_ind_old_to_new(link_label + 1);
%     end
    link_label = input_graph.tmp_link_ind_old_to_new(link_label + 1);
    while ~isempty(link_label) && (link_label ~= 0)
        % Since this is the link connected to an internal endpoint, only one node is connected
        % Check the length of the link, since the length of the link can
        % change during pruning
